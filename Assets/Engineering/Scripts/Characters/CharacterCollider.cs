...
{
    public static CharacterCollider Instance { get; private set; }
    ...
    protected const int k_FinalLayerIndex = 0;
    protected const int k_PortalLayerIndex = 11;
    protected const float k_DefaultInvinsibleTime = 2f;

    protected const int k_FuelAddLayerIndex = 12;
    ...
    protected void OnTriggerEnter(Collider c)
    {
        ...
        else if(c.gameObject.layer == k_ObstacleLayerIndex)
        {
			...
			c.enabled = false;
			if (controller.reverseControl)
			{
				controller.reverseWorld = true;
			}
        ...
        // if player colliding with winning object
        else if (c.gameObject.layer == k_FinalLayerIndex)
        {
	        controller.StopMoving();
	        c.enabled = false;
	        if (controller.reverseControl)
        ...
        else if (c.gameObject.layer == k_PortalLayerIndex)
        {
	        controller.reverseWorld = true;
        }
        
		// if player collide with Fuel Add Props
	   
		else if(c.gameObject.layer == k_FuelAddLayerIndex) 
		{
            Obstacle ob = c.gameObject.GetComponent<Obstacle>();
           
			m_Audio.PlayOneShot(ob.impactedSound);
            controller.AddFuel();
