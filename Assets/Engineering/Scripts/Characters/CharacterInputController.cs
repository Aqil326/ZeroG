...
public class CharacterInputController : MonoBehaviour
{
	...
	public bool isWin { get { return m_isWin;} set { m_isWin = value; } }
	[Header("Controls")]
	...
	protected bool m_Thrusting;

	protected bool m_landed = true;
    ...
    private bool uiThrustButton = false;
    // Declare a variable to track tap duration
    ...
    protected void Awake ()
    {
        ...
        {
            Debug.LogWarning("Multiple instances of CharacterInputController found. Ensure only one exists.");
            Destroy(gameObject);
        }
	...
	public void Begin()
	{
		// husnain add this code for start geme from sky to ground with thrust effect
        if (TrackManager.instance.m_CurrentLevel > 1)
        {
           // new Vector to adjust the position of player in start of level 2 or above
            characterCollider.transform.localPosition = new Vector3(0, 2.2f, 0);
		  //  CharacterCollider.Instance.thrustParticle.Play();
            StartCoroutine(StopThrustParticleAfterDelay(1.2f));
        }
        else
        {
            characterCollider.transform.localPosition = Vector3.zero;
    ...
    private IEnumerator StopThrustParticleAfterDelay(float delay)
    {
        yield return new WaitForSeconds(delay);
        CharacterCollider.Instance.thrustParticle.Stop();
		CharacterCollider.Instance.dustParticle.Play();
    }
	...
	protected void Update ()
    {
		...
		else if (Input.GetKeyDown(KeyCode.DownArrow) && TutorialMoveCheck(1))
        {
            if (reverseControl)
            {
                Jump();
		...
		else if (Input.GetKeyDown(KeyCode.Space) && TutorialMoveCheck(3))
        {
            ThrustSetup();
            character.animator.SetBool(s_MovingHash, false);
            Debug.Log("Still in TMC 3............");
        }

        else if (Input.GetKeyDown(KeyCode.UpArrow) && TutorialMoveCheck(2))
        {
	        if (reverseControl)
	        {
		        Slide();
...
if (Input.touchCount == 1)
{
    ...
    else if (touch.phase == TouchPhase.Moved || touch.phase == TouchPhase.Stationary)
    {
        ...
        if (m_IsSwiping)
        {
            ...
            if (diff.magnitude > 0.1f && !reverseControl)
            {
                // Swipe detection logic (same as in your original code)
            ...
            else if (diff.magnitude > 0.1f && reverseControl)
            {
                // Swipe detection logic (same as in your original code)
                if (Mathf.Abs(diff.y) > Mathf.Abs(diff.x))
                {
            ...
            {   
			    if(TutorialMoveCheck(3))
				{	
						
	            if (tapDuration > 0.25f && !m_Thrusting)
	            {
		             ThrustSetup();
		             m_IsSwiping = false;
	            }
    ...
    {
        // Check if the touch ended without a swipe
        if (m_IsSwiping)
        {
            m_IsSwiping = false;
        }
   ...
   //      }
#endif
        // this function should call for every type of input if button is pressed
        if (uiThrustButton)
	    {
		    Thrust();
	    }
		...
		if (!m_Thrusting && !m_Jumping && m_TargetPosition.y > 0)
		{
			m_TargetPosition.y -= thrustSpeed * Time.deltaTime;
			if (m_TargetPosition.y < 0.2f)
			{
				m_landed = true;
			}
		}

        if(m_Jumping && !m_Thrusting)
        {
			if (trackManager.isMoving)
			{
				...
				{
					StopJumping();
					characterCollider.dustParticle.Play();
				}
				else
				{
	...
	public void ThrustSetup()
	{
		if (trackManager.Fuel<=0)
		{
			return;
		}
//		Debug.Log("Calling thrust setup");
		if (!m_IsRunning)
			return;
		tapDuration = 0f;
		// if (!m_Jumping)
		...
		StopJumping();
		//characterCollider.dustParticle.Play();
		characterCollider.thrustParticle.Play();

		if (m_TargetPosition.y < maxThrust)
		{
			m_TargetPosition.y = characterCollider.transform.localPosition.y;
		}
		// float animSpeed = k_TrackSpeedToJumpAnimSpeedRatio * (trackManager.speed / jumpLength);
		//
		// character.animator.SetFloat(s_JumpingSpeedHash, animSpeed);
		// character.animator.SetBool(s_JumpingHash, true);
		// have to do something with audio
...
//		Debug.Log("Calling thrust stop");
		tapDuration = 0f;
		characterCollider.thrustParticle.Stop();

		uiThrustButton = false;
		m_Thrusting = false;
	}
	public void Thrust()
	{
		//Debug.Log("Calling thrust");
		if (!m_IsRunning)
			return;
		if (trackManager.Fuel <= 0)
		{
	...
	public void ChangeLane(int direction)
    {
        ...
        m_TargetPosition = new Vector3((m_CurrentLane - 1) * trackManager.laneOffset, m_TargetPosition.y, 0);
    }
