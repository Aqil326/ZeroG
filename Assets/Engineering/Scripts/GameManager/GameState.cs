...
public class GameState : AState
{
    ...
    static int s_WinHash = Animator.StringToHash("isWin");
    ...
    {
        trackManager.characterController.isWin = false;
    ...
    public void StartGame()
    {
        ...
        if (m_IsTutorial)
        {  
            // I set current level 4, because i want to run four patches 
            trackManager.CurrentLevel = 4;
        ...
        if (trackManager.m_CurrentLevel > 2 && !m_IsTutorial)
        {
    ...
    public override void Tick()
    {
        ...
        if (trackManager.isLoaded)
        {
            ...
            // if player wins change the state from gameover to gamewin
            if (chrCtrl.isWin)
            {
                cameraManager.Shake();
                pauseButton.gameObject.SetActive(false);
                chrCtrl.CleanConsumable(); 
                //chrCtrl.character.animator.SetTrigger(s_WinHash);
            ...
            // reversing world if true
            if (chrCtrl.reverseWorld)
            {
                UpsideDownWorldSwitch();
                chrCtrl.reverseControl = !chrCtrl.reverseControl;
                chrCtrl.reverseWorld = false;
            }
    ...
    protected void UpdateUI()
    {
        ...
        LevelNumberText.text = "Level : " + trackManager.CurrentLevel;
        fuelSlider.value = trackManager.Fuel;
        fuelSlider.image.color = Color.Lerp(Color.red, Color.white, fuelSlider.value / 5);
		for (int i = 0; i < 3; ++i)
		{
	...
	IEnumerator WaitForGameOver()
	{
        ...
        if (currentModifier.OnRunEnd(this))
        {
            ...
            // This is the case where we are checking if player dies first time so, it shouldn't be redirect
            // to gameover screen directly, else it should show a re-run chance if his currency is greater than threashold
            // else
            //     OpenGameOverPopup();
        }
	}
    ...
    void TutorialCheckObstacleClear()
    {
        ...
        if (m_CountObstacles && ratio > nextObstaclePosition + 0.05f)
        {
          ...
          //  if (m_TutorialClearedObstacle == k_ObstacleToClear)
            //if (m_TutorialClearedObstacle == 1)


            {
                Debug.Log(m_TutorialClearedObstacle);
                ...
                {//we looped, mean we finished the tutorial.
                    trackManager.characterController.currentTutorialLevel = 4;
                    DisplayTutorial(true);
                }

            }
        }
        else if (m_DisplayTutorial && ratio > nextObstaclePosition - 0.5f && trackManager.currentZone < 2)
        { DisplayTutorial(true); }

        else if (m_DisplayTutorial && ratio > nextObstaclePosition - 0.9f && trackManager.currentZone == 3)
        {
    ...
    public async Task UpsideDownWorldSwitch()
    {
        float currentSpeed = trackManager.speed;
        trackManager.speed = 0;
        cameraManager.UpsideDownCamera();
        await Task.Delay(200);
        trackManager.speed = (currentSpeed / 2);
    }
