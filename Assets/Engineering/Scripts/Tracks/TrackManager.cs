...
public class TrackManager : MonoBehaviour
{
    ...
    static int s_JumpHash = Animator.StringToHash("newJump");
    ...
    public int TotalLevel { get { return m_TotalLevel; } set { m_TotalLevel = value; } }
    ...
    protected const float k_CountdownToStartLength = 3f;
    ...
    // variable to check either zones have completed or not
    // if yes then it's time to spawn winning obstacle
    // also isWinningObsSpawned checking either winning obstacle is spawnned or not
    private bool isZonesCompleted = false;
    private bool isWinningObsSpawned = false;

    // Portal related Data
    private int maxPortalsCanSpawn;     // max portals can spawn in a level
    private int portalsSpawned;     // max portals can spawn in a level
    private int beforePortalObstacleRatio = 6;     // spawn 10 obstacles before portal
    private int obstaclesBeforePortal = 0;     // managing count of obstacles before portal spawning
    private int k_portalLayer = 11;

    private int obstaclesBeforeFuel = 0;
    
    private int m_randomObstacle;   // check not to spawn type of obstacle again
    ...
    IEnumerator WaitToStart()
    {
        ...
        {
            characterController.character.animator.Play(s_JumpHash);
        }
        ...
        while (m_TimeToStart >= 0)
        {
         ...
         //   m_TimeToStart -= Time.deltaTime * k_CountdownSpeed;
            m_TimeToStart -= Time.deltaTime * 2.5f;
        }
    ...
    {
        // at the begning level 3 managing fuel value
        m_Fuel = 0.0f;
        ...
        // Setting max speed according to it's level
        maxSpeed = m_CurrentLevel * 2;
        
        if (maxSpeed > maxSpeedThreshold)
        {
            maxSpeed = maxSpeedThreshold;
        }
        else if (maxSpeed < minSpeedThreshold)
        {
            maxSpeed = minSpeedThreshold;
        }
        // Here it is managing that how many portals can spawn depending upon current level
        if (m_CurrentLevel < 3)
        {
            maxPortalsCanSpawn = 0;
        }
        else
        {
            maxPortalsCanSpawn = m_CurrentLevel - 2;
            if (maxPortalsCanSpawn > 5)
            {
                maxPortalsCanSpawn = 5;
            }

            portalsSpawned = 0;
            obstaclesBeforePortal = 0;
        }
        ...
        if (!m_Rerun)
        {
            ...
            m_TotalLevel = m_CurrentThemeData.zones.Length;
            m_CurrentZone = 0;
            m_CurrentZoneDistance = 0;
            isWinningObsSpawned = false;
            isZonesCompleted = false;
        ...
        //This function is basically doesn't just start the game immediatly 
        // it waits for few seconds and then start the game 
        StartCoroutine(WaitToStart());
        // calling below two functions for subsitiute
        // characterController.StartRunning();
        // StartMove();
        isLoaded = true;
    }
...
//             PlayerData.instance.UpdateMissions(this);
    ...
    public void ChangeZone()
    {
        ...
        //    m_CurrentZone = 0;
        // Here the user will win the game if there is no more zone left to play

        if (m_CurrentZone >= m_CurrentLevel)
        {
            m_CurrentZone = 0;
            // Here write some logic to spawn winning obstacle
            isZonesCompleted = true;
            //            Debug.Log("Zones completed");
        }
    ...
    public IEnumerator SpawnNewSegment()
    {
        ...
        // Here it is spawning randomly but in our game I will spawn this in an order
        ...
        if (m_SafeSegementLeft <= 0)
        {
            ...
            if (isZonesCompleted && !isWinningObsSpawned)
            {

             //Debug.Log("Spawninig winning obstacle");
             SpawnWinningObstacle(newSegment); 
    ...
    #region spawn obstacles old code
    //public void SpawnObstacle(TrackSegment segment)
    //{

    //    if (isWinningObsSpawned) return;

    //    if (segment.possibleObstacles.Length != 0)
    //    {
    //        // spawning each obstacle in each new level
    //        //  int obstaclesToSpawn = m_CurrentLevel * 2;
    //        int obstaclesToSpawn;

    //        if (m_CurrentLevel >= 5)
    //        {
    //            obstaclesToSpawn = Mathf.Min(8, segment.possibleObstacles.Length);
    //        }

    //        else if (m_CurrentLevel == 4)
    //        {
    //            obstaclesToSpawn = Mathf.Min(7, segment.possibleObstacles.Length);
    //        }
    //        else
    //        {
    //            obstaclesToSpawn = Mathf.Min(Mathf.Min(m_CurrentLevel * 2, segment.possibleObstacles.Length), 6);
    //        }

    //        for (int i = 0; i < segment.obstaclePositions.Length; ++i)
    //        {
    //            if (obstaclesToSpawn > segment.possibleObstacles.Length)
    //            {
    //                obstaclesToSpawn = segment.possibleObstacles.Length;
    //            }

    //            if (obstaclesBeforeFuel > 6 && m_CurrentLevel > 2)
    //            {

    //                FuelAdd(segment);
    //                obstaclesBeforeFuel = 0;
    //                //Debug.Log("Fuel Spawn");
    //            }


    //            else if (obstaclesBeforePortal > beforePortalObstacleRatio)
    //            {

    //                SpawnPortal(segment);
    //                obstaclesBeforePortal = 0;
    //                //Debug.Log("Porta; Spawn");
    //            }
    //            else
    //            {
    //                obstaclesBeforePortal++;
    //                if (m_CurrentLevel > 2)
    //                { obstaclesBeforeFuel++; }

    //                //Debug.Log("Obstacles Spawn");
    //                // check is it a portal or not
    //                // stopping from spawning same obstacle again and again

    //                int newRandomObstacleNumber = Random.Range(0, obstaclesToSpawn);

    //                while (m_randomObstacle == newRandomObstacleNumber)
    //                {
    //                    newRandomObstacleNumber = Random.Range(0, obstaclesToSpawn);
    //                }

    //                m_randomObstacle = newRandomObstacleNumber;

    //                AssetReference assetRef = segment.possibleObstacles[m_randomObstacle];
    //                StartCoroutine(SpawnFromAssetReference(assetRef, segment, i));

    //            }
    //        }
    //    }
    //    // Stop spawning powerup and coins
    ...
    public void SpawnObstacle(TrackSegment segment)
    {
        ...
        if (segment.possibleObstacles.Length != 0)
        {
            ...
            bool avoidBigObstacleConsecutive = false;
            if (m_CurrentLevel >= 4 && obstaclesToSpawn > 1)
            {
                avoidBigObstacleConsecutive = true;
              //  obstaclesToSpawn--;
            }

            int bigObstacleIndex = -1;
            ...
            for (int i = 0; i < segment.obstaclePositions.Length; ++i)
            {
                ...
                if (obstaclesBeforeFuel > 5 && m_CurrentLevel > 2)
                {
                ...
                else
                {
                    ...
                    if (m_CurrentLevel >= 5)
                    {
                        ...
                        do
                        {
                            ...
                            while (m_randomObstacle == randomNumber)
                            {
                                randomNumber = Random.Range(0,8);
                            }

                            m_randomObstacle = randomNumber;
                        } while (randomNumber == 6);



                        //if (avoidBigObstacleConsecutive && bigObstacleIndex >= 0)
                        //{
                        //    m_randomObstacle = bigObstacleIndex < segment.possibleObstacles.Length - 1 ? bigObstacleIndex + 1 : 0;
                        //}
                        //else
                        //{

                        //    int newRandomObstacleNumber2 = Random.Range(0, obstaclesToSpawn);

                        //    while (m_randomObstacle == newRandomObstacleNumber2)
                        //    {
                        //        newRandomObstacleNumber2 = Random.Range(0, obstaclesToSpawn);
                        //    }

                        //    m_randomObstacle = newRandomObstacleNumber2;
                        //  //  m_randomObstacle = Random.Range(0, obstaclesToSpawn);
                        //    if (avoidBigObstacleConsecutive)
                        //    {
                        //        bigObstacleIndex = m_randomObstacle;
                        //    }
                        //}
                    }
    ...
    private void FuelAdd(TrackSegment segment)
    {
            if(!m_IsTutorial)
        {
            AssetReference assetRef = segment.fuelAdd;
            StartCoroutine(SpawnFromAssetReference(assetRef, segment, 0));
        }
           
        
    }

    private void SpawnPortal(TrackSegment segment)
    {
        if (segment.FinalObstacle != null && maxPortalsCanSpawn > portalsSpawned && !m_IsTutorial)
        {
            AssetReference assetRef = segment.portalObstacle;
            StartCoroutine(SpawnFromAssetReference(assetRef, segment, 0));
            portalsSpawned++;
        }
    }
    private void SpawnWinningObstacle(TrackSegment segment)
    {
         
        if (segment.FinalObstacle != null && !m_IsTutorial)
        {
            AssetReference assetRef = segment.FinalObstacle;
            StartCoroutine(SpawnFromAssetReference(assetRef, segment, 0));
            isWinningObsSpawned = true;
        }
    }
    ...
    private IEnumerator SpawnFromAssetReference(AssetReference reference, TrackSegment segment, int posIndex)
    {
        ...
        if (obj != null)
        {
            ...
            if (obstacle != null)
                yield return obstacle.Spawn(segment, segment.obstaclePositions[posIndex], obj);
        }
    ...
    public void UpdateLevel()
    {
        ...
        {
            m_CurrentLevel = Random.Range(1, m_TotalLevel);
        }
        
        // Here we can add some logic to change the theme
    }
    
    // This function will decrease fuel
    public void ConsumeFuel()
    {
        if (m_Fuel>0)
        {
            m_Fuel -= fuelConsumptionRate * Time.deltaTime;
        }
    }
